...
namespace PruductBundle\Controller;


use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use PruductBundle\Entity\Panier;
use PruductBundle\Entity\Produit;
...
class PanierController extends Controller
{
    ...
    public function reserverAction(Request $request)
    {
        ...
        $a=$em->getRepository('PruductBundle:Produit')->find($id);
        ...
        if ($request->isMethod('post')) {
            ...
            if($a->getQuantite() >= $request->get('nbre')) {
               ...
               $nb = $a->getQuantite() - $nbre;
                /* @var $entity \PruductBundle\Entity\Produit */
               $a=$em->getRepository('PruductBundle:Produit')
                   ->decrementer($id,$nb);
    ...
    public function indexAction()
    {
       ...
       // $pan = $em->getRepository("PruductBundle:Panier")
        ...
        $pan = $em->getRepository('PruductBundle:Panier')->findAll();
    ...
    public function annulerAction(Request $request)
    {
        ...
        $em=$this->getDoctrine()->getManager();
        $res = $em->getRepository("PruductBundle:Panier")->find($id);
        $a=$em->getRepository('PruductBundle:Produit')->find($res->getProduit()->getId());

        $a=$em->getRepository('PruductBundle:Produit')
            ->incrementer($res->getProduit()->getId(),$res->getQunantite()+$a->getId());

        $t=$em->getRepository('PruductBundle:Panier')->find($id);
        $em->remove($t);
        $em->flush();
